...
class ProductController extends PublicController {
    ...
    public function index(){
		...
		$content = str_replace('/miniguangzhoudaguansi/Data/', __DATAURL__, $pro['content']);
	...
	public function details(){
		...
		$pro = M('product')->where('id='.intval($ls_id).' AND del=0')->find();
		if(!$pro){
		...
		M('product')->where('id='.intval($ls_id))->save($up);
    ...
    public function category(){
    	$lslist = M('product')->where('del=0')->select();
    	foreach ($lslist as $k => $v) {
    		$lslist[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
    	}
    	echo json_encode(array('lslist'=>$lslist));
 		exit();
    }

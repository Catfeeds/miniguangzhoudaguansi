...
class UserController extends PublicController {
	...
	public function userinfo(){
		...
		if($user['bl_photo']){
			$user['bl_photo'] = __DATAURL__.$user['bl_photo'];
		}
		if($user['photo_ls']){
			$user['photo_ls'] = __DATAURL__.$user['photo_ls'];
		}
	...
	public function my_supply(){
		...
		$supply = M('supply');
		$state = array('0'=>'咨询中','1'=>'有人接单','2'=>'已取消','3'=>'达成合作');
	...
	public function uploadls(){
		...
		$info = $this->upload_images($_FILES['img'],array('jpg','png','jpeg'),"ls_photo/".date(Ymd));
		if(is_array($info)) {// 上传错误提示错误信息
			$url = 'UploadFiles/'.$info['savepath'].$info['savename'];
			//修改会员图片
			$up = M('user')->where('id='.intval($uid))->save(array('photo_ls'=>$url));
			if ($up) {
				$xt = $check['photo_ls'];
				if ($uid && $xt) {
					$img_url = "Data/".$xt['photo_ls'];
					if(file_exists($img_url)) {
						@unlink($img_url);
					}
				}
	...
	*/
    ...
    public function get_code(){
        ...
        if(!preg_match("/^13[0-9]{1}[0-9]{8}$|15[05189]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$/",$tel)){    

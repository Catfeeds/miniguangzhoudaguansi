...
class ShangchangController extends PublicController {
    ...
    public function lists(){
        ...
        //获取所有的商家数据
        $store_list = M('shangchang')->where($condition)->order('sort asc,addtime desc')->limit($limit.',8')->select();
        foreach ($store_list as $k => $v) {
            $store_list[$k]['logo'] = __DATAURL__.$v['logo'];
            $store_list[$k]['addtime'] = date("Y-m-d",$v['addtime']);
        }
   ...
   public function shop_details(){
        ...
        $content = str_replace("/miniguangzhoudaguansi/Data/", __DATAURL__, $shop_info['content']);
        $content=html_entity_decode($content, ENT_QUOTES ,'utf-8');
        echo json_encode(array('status'=>1,'shop_info'=>$shop_info,'content'=>$content,'tel'=>$tel));
        exit();
    }

...
class UserController extends PublicController{
	...
	public function shenhe(){
		...
		if ($res) {
			$data = array();
			$temp = M('user')->where('id='.intval($id))->find();
			$data['name'] = $temp['truename'];
			$data['photo'] = $temp['bl_photo'];
			$data['tel'] = $temp['tel'];
			$data['shop_name'] = $temp['shop_name'];
			$data['dizhi'] = $temp['dizhi'];
			$data['service'] = $temp['service'];
			$data['cz'] = $temp['cz'];
			$data['photo_x'] = $temp['photo_ls'];
			$data['zyjy'] = $temp['zyjy'];
			$data['digest'] = $temp['digest'];
			$data['uid'] = $id;
			$data['addtime'] = time();
			$res2 = M('product')->add($data);
			if($res2){
				...
				$tmp['type'] = 1;
				$tmp['reason'] = '';
				M('user')->where('id='.intval($id))->save($tmp);
				$this->error('操作失败！');
				exit();
			}
